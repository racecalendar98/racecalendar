...
let selectedSeason = Number(new URLSearchParams(window.location.search).get('season')) || 2026;
...
seasonSelect.addEventListener('change', () => {
  ...
  if(selectedSeason === 2027 && !location.pathname.endsWith('/2027.html')){
    location.href = new URL(`2027/index.html?series=${encodeURIComponent(activeSeries.id)}`, location.href).href;
    return;
  }
  viewYear = selectedSeason;
  viewMonth = 0;
...
function showHome(){
  ...
  document.getElementById('seriesView').hidden = true;
...
function renderHero(){
  ...
  metaEl.innerHTML = `<span class="series-tag">${activeSeries.name}</span> · Round ${race.round} · ${race.circuit} · ${fmtDateJP(race.dateObj)}`;
  const sessions = buildSessions(activeSeries, race, race.dateObj);
  sessionsEl.innerHTML = `<span class="hero-sessions-title">TIME SCHEDULE</span>${sessions.map(session => `<div class="hero-session"><span>${session.label.replace('フリープラクティス', 'フリー走行')}</span><strong>${fmtDateJP(session.date)} ${String(session.time || '未定').replace(/\s*JST$/, '')}</strong></div>`).join('')}`;
  if(countdownTimer) clearInterval(countdownTimer);
  function tick(){
...
function renderList(){
  ...
  activeSeries.races.filter(isRaceInSelectedSeason).forEach(r=>{
    ...
    card.innerHTML = `
      <div class="race-round"><strong>${String(r.round).padStart(2,'0')}</strong>ROUND</div>
      ...
      <div class="race-date">${fmtDateJP(d)}</div>
    `;
...
function renderStandings(){
  ...
  document.getElementById('standingsNote').textContent = isManufacturerMode
    ...
    : (isTeamMode ? 'チームランキング' : (activeSeries.standingsNote || '公式順位が確定次第、ここに表示します。'));
  const table = document.getElementById('standingsTable');
  table.innerHTML = '';
  const groups = new Map();
  const sourceRows = isManufacturerMode ? activeSeries.manufacturerStandings : (isTeamMode ? (activeSeries.teamStandings || buildTeamStandings(activeSeries.standings || [])) : (activeSeries.standings || []));
  ...
  groups.forEach((rows, className) => {
    ...
    combineCrewStandings(rows).sort((a,b)=>(b.points-a.points) || (a.pos-b.pos)).forEach((row, index)=>{
      ...
      const secondary = isTeamMode ? (getTeamDrivers(row) || row.team) : row.team;
...
function getTeamDrivers(teamRow){
  ...
  const sourceClass = String(teamRow.class || '').replace(/\s*チーム$/, '');
  const drivers = [teamRow.teamDrivers, ...(activeSeries.standings || [])
    .filter(row => row.class === sourceClass && row.car === teamRow.car)
...
function openRaceModal(race){
  ...
  document.getElementById('modalSub').textContent = `${activeSeries.name} · Round ${race.round} · ${race.circuit}`;
  document.querySelector('.modal-section-label').textContent = 'セッションスケジュール';
  const list = document.getElementById('modalSessions');
  list.innerHTML = '';
  sessions.forEach(s=>{
    const row = document.createElement('div');
    row.className = 'session-row';
    row.innerHTML = `<span class="label">${s.label.replace('フリープラクティス', 'フリー走行')}</span><span class="time">${fmtDateJP(s.date)} ${String(s.time || '未定').replace(/\s*JST$/, '')}</span>`;
