...
function upcomingRaces(){
  return SERIES.flatMap(series => series.races.map(race => ({ ...race, series, dateObj:parseDate(race.date) })))
    .filter(race => race.dateObj >= TODAY)
...
function homeRaceUrl(item){
  return RaceSeo.urlFor(item.series.id, item.round, 2027);
}

function homeRaceMarkup(item, featured){
  const days = Math.max(0, Math.ceil((item.dateObj - TODAY) / 86400000));
  const style = `--home-color:var(${item.series.color})`;
  if(featured){
    const diff = Math.max(0, item.dateObj - liveNow());
    ...
    const sessionMarkup = `<div class="hero-sessions home-sessions" aria-label="タイムスケジュール"><span class="hero-sessions-title">TIME SCHEDULE</span>${sessions.map(session => `<div class="hero-session"><span>${session.label}</span><strong>${fmtDateJP(session.date)} ${session.time || '未定'}</strong></div>`).join('')}</div>`;
...
function renderHome(){
  ...
  paintLights('#homeFeatured', lightsForDays(Math.max(0, Math.ceil((races[0].dateObj - TODAY) / 86400000))), races[0].series.color);
